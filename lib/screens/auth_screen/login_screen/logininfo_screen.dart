    ...
    try {
      ...
      if (userDoc.exists) {
        ...
        if (storedPassword == password) {
          ...
          // Auto-register user with backend (if not already registered)
          // This will fetch data from Firestore and send to backend
          // Backend handles duplicate check - if user exists, nothing happens
          _backendService.autoRegisterCurrentUser().then((userId) {
            if (userId != null) {
              debugPrint('✅ User registered with backend: $userId');
            } else {
              debugPrint('ℹ️ User already registered or registration skipped');
            }
          }).catchError((error) {
            debugPrint('⚠️ Backend registration failed (non-critical): $error');
          });
