...
class ContactsScreen extends ConsumerWidget {
                            ...
                            ElevatedButton(
                              ...
                              onPressed: () async {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                      content: Text(
                                          'Calling disabled in this build')),
                                );
                              },
                              child: const Text(
                                'CALL 9999',
  ...
  Future<void> _removeContact(BuildContext context, WidgetRef ref, User user,
    ...
    try {
      ...
      // Remove contact from current user's emergency_contacts
      await user.documentRef.update({
        'emergency_contacts': FieldValue.arrayRemove([contact.toFirestore()]),
      });

      // CRITICAL FIX: Remove current user from contact's emergency_contact_of field
      ...
      if (currentUserPhone != null) {
        ...
        await contactDocRef.update({
          'emergency_contact_of': FieldValue.arrayRemove([currentUserPhone]),
        }).catchError((e) {
          print('Error updating contact emergency_contact_of: $e');
          // Contact might not exist in system
        });
      }
            ...
            : () async {
                ...
                if (selectedContact != null) {
                  ...
                  try {
                    ...
                    // Add contact to current user's emergency_contacts
                    await user.documentRef.update({
                      'emergency_contacts':
                          FieldValue.arrayUnion([emergencyContact.toFirestore()]),
                    });

                    // CRITICAL FIX: Create contact document if it doesn't exist
                    if (currentUserPhone != null) {
                      final contactDocRef = FirebaseFirestore.instance
                          .collection('users')
                          .doc(emergencyContact.number);

                      // First, check if contact document exists
                      final contactDoc = await contactDocRef.get();
                      
                      if (!contactDoc.exists) {
                        // Create minimal contact document if it doesn't exist
                        await contactDocRef.set({
                          'phoneNumber': emergencyContact.number,
                          'name': emergencyContact.name,
                          'emergency_contact_of': [currentUserPhone],
                        });
                        print('Created new contact document for ${emergencyContact.number}');
                      } else {
                        // Update existing contact's emergency_contact_of field
                        await contactDocRef.update({
                          'emergency_contact_of':
                              FieldValue.arrayUnion([currentUserPhone]),
                        });
                        print('Updated existing contact document for ${emergencyContact.number}');
                      }
                    }
