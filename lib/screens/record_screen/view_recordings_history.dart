...
class ViewRecordingsHistory extends StatefulWidget {
  ...
  const ViewRecordingsHistory({super.key, this.userID});
...
class _ViewRecordingHistoryState extends State<ViewRecordingsHistory> {
  ...
  Future<void> _initializeUserID() async {
    try {
      // Prefer the userID passed in from the parent widget
      String? resolvedID = widget.userID;

      if (resolvedID == null) {
        final SharedPreferences prefs = await SharedPreferences.getInstance();
        resolvedID = prefs.getString('phoneNumber');
      }
  ...
  Future<void> fetchDateFolders() async {
    if (userID.isNotEmpty) {
      try {
        final imageFolders =
            await LocalStorageService.listImageDateFolders(userID);
        final audioFolders =
            await LocalStorageService.listAudioDateFolders(userID);
        final videoFolders =
            await LocalStorageService.listVideoDateFolders(userID);

        // Merge, de-duplicate and sort by date (newest first)
