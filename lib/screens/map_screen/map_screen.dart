...
import '../../services/debug/debug_track_me.dart';
import '../../services/debug/fix_emergency_contact_of.dart';
import '../../services/background/background_services.dart';
import '../tracking_screen/track_emergency_contact_location_map.dart';
...
class MapScreenState extends ConsumerState<MapScreen> {
  ...
  Future<void> _checkForAlerts() async {
    final alertData = await ManualAlertCheckService.checkForTrackMeAlerts(context);
    
    if (alertData != null) {
      // Show success message
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('📍 ${alertData['contactName']} is sharing location!'),
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (context) => TrackEmergencyContactLocationMap(
            panickedPersonName: alertData['contactName'],
            panickedPersonProfilePic: alertData['contactProfilePic'] ?? 'assets/placeholders/default_profile_pic.png',
            panickedPersonAlertDetails: alertData['alert'],
          ),
        ),
      );
    }
                  ...
                  children: alertsWithContacts.map((alertWithContact) {
                    if (alertWithContact.alert.alertType == 'trackMe') {
                      ...
                      return NotificationWidget(
                        ...
                        panickedPersonAlertDetails: alertWithContact.alert,
                        panickedPersonPhone: alertWithContact.contactPhone,
                      );
                    }
          ...
          if (kDebugMode)
            Positioned(
              ...
              child: FloatingActionButton(
                onPressed: () async {
                  // First fix the relationship
                  await FixEmergencyContactOf.fixBidirectionalRelationship();
                  // Then show debug info
                  await DebugTrackMe.debugTrackMeSetup();
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('✅ Fixed! Check console for details'),
                      backgroundColor: Colors.green,
